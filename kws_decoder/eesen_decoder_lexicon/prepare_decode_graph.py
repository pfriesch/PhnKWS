...
from kws_decoder.build_1_gram_fst import build_1_gram_fst
from kws_decoder.build_grammar_fst import build_grammar_fst
...
def filter_lexicon(libri_lexicon, out_lexicon):
    lines = [f"{SIL_SYM} SIL\n",
             f"{SPN_SYM} SPN\n",
             f"{UNK_SYM} SPN\n"]
...
def make_ctc_decoding_graph(arpa_lm_path, phn2idx, tmpdir,
                            lexicon_path=f"{KALDI_ROOT}/egs/librispeech/s5/data/local/lm/librispeech-lexicon.txt",
                            draw_G_L_fsts=False, no_lm=False):
    ...
    filter_lexicon(lexicon_path, f"{tmpdir}/lexicon.txt")
    ...
    with open(f"{graph_dir}/lexicon_fst.txt", "w") as f:
        f.writelines(lexicon_fst)
    del lexicon_fst

    run_shell(f"fstcompile --isymbols={graph_dir}/tokens.txt --osymbols={graph_dir}/words.txt "
              + f"--keep_isymbols=false --keep_osymbols=false {graph_dir}/lexicon_fst.txt | "
    ...
    if no_lm:
        grammar_fst_path = build_1_gram_fst(arpa_lm_path, graph_dir)
    else:
        grammar_fst_path = build_grammar_fst(arpa_lm_path, graph_dir)
    if not grammar_fst_path == f"{graph_dir}/G.fst":
