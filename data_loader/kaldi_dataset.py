    ...
    def __init__(self, feature_dict, label_dict, context_left, context_right, max_sequence_length, tensorboard_logger,
        ...
        if debug:
            for label_name in _label_dict:
                _label_dict[label_name] = dict(sorted(list(_label_dict[label_name].items()), key=lambda x: x[0])[:30])
            for feat_name in _feature_dict:
                _feature_dict[feat_name] = dict(sorted(list(_feature_dict[feat_name].items()), key=lambda x: x[0])[:30])
