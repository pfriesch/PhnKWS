            ...
            with tqdm(disable=not logger.isEnabledFor(logging.INFO), total=len(dataloader), position=0) as pbar:
                ...
                for batch_idx, (_, inputs, targets) in enumerate(dataloader):
                    ...
                    if self.config['exp']['compute_train_metrics']:
                        _train_metrics_future = metrics_pool.apply_async(
                            eval_metrics, (detach_metrics(output, targets), self.metrics))
                        accumulated_train_metrics.append(_train_metrics_future)
                        total_train_metrics.append(_train_metrics_future)
                    ...
                    if (time.time() - last_train_logging) > 30:
                        ...
                        if self.config['exp']['compute_train_metrics']:
                            ...
                            for metric in [m.get() for m in tqdm(accumulated_train_metrics,
                                                                 desc="get metrics", position=1)]:
                                for metric, metric_value in metric.items():
                                    train_metrics[metric] += metric_value
                    ...
                    #### /Logging ####
            train_metrics = {metric: 0 for metric in self.metrics}
            for metric in [m.get() for m in tqdm(total_train_metrics, desc="get metrics ")]:
                for metric, metric_value in metric.items():
                    train_metrics[metric] += metric_value
            ...
            if self.config['exp']['compute_train_metrics']:
                for metric in train_metrics:
                    self.tensorboard_logger.add_scalar(metric + "_avg",
                                                       train_metrics[metric] / n_steps_this_epoch)
        ...
        with Pool(processes=2) as metrics_pool:
            with tqdm(disable=not logger.isEnabledFor(logging.INFO), total=len(dataloader)) as pbar:
                ...
                for batch_idx, (_, inputs, targets) in enumerate(dataloader):
                    ...
                    _valid_metrics_future = metrics_pool.apply_async(
                        eval_metrics, (detach_metrics(output, targets), self.metrics))
                    accumulated_valid_metrics.append(_valid_metrics_future)

                    pbar.set_description('V e:{} l: {:.4f} '.format(epoch, loss["loss_final"].item()))
                    pbar.update()
                    #### /Logging ####
            valid_metrics = {metric: 0 for metric in self.metrics}
            for metric in [m.get() for m in tqdm(accumulated_valid_metrics, desc="get metrics")]:
                for metric, metric_value in metric.items():
