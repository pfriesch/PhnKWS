...
class WriterTensorboardX(SummaryWriter):
    ...
    def add_scalar(self, tag, scalar_value, global_step=None, walltime=None, mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_scalar('{}/{}'.format(mode, tag), scalar_value, global_step, walltime)
    ...
    def add_scalars(self, main_tag, tag_scalar_dict, global_step=None, walltime=None, mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_scalars('{}/{}'.format(mode, main_tag), tag_scalar_dict, global_step, walltime)
    ...
    def add_histogram(self, tag, values, global_step=None, bins='tensorflow', walltime=None, mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_histogram('{}/{}'.format(mode, tag), values, global_step, bins, walltime)
    ...
    def add_image(self, tag, img_tensor, global_step=None, walltime=None, dataformats='CHW', mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_image('{}/{}'.format(mode, tag), img_tensor, global_step, walltime, dataformats)
    ...
    def add_images(self, tag, img_tensor, global_step=None, walltime=None, dataformats='NCHW', mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_images('{}/{}'.format(mode, tag), img_tensor, global_step, walltime, dataformats)
    ...
    def add_image_with_boxes(self, tag, img_tensor, box_tensor, global_step=None, walltime=None, dataformats='CHW',
        ...
        if mode is None:
            mode = self.mode
        super().add_image_with_boxes('{}/{}'.format(mode, tag), img_tensor, box_tensor, global_step, walltime,
                                     dataformats, **kwargs)
    ...
    def add_figure(self, tag, figure, global_step=None, close=True, walltime=None, mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_figure('{}/{}'.format(mode, tag), figure, global_step, close, walltime)
    ...
    def add_video(self, tag, vid_tensor, global_step=None, fps=4, walltime=None, mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_video('{}/{}'.format(mode, tag), vid_tensor, global_step, fps, walltime)
    ...
    def add_audio(self, tag, snd_tensor, global_step=None, sample_rate=44100, walltime=None, mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_audio('{}/{}'.format(mode, tag), snd_tensor, global_step, sample_rate, walltime)
    ...
    def add_text(self, tag, text_string, global_step=None, walltime=None, mode=None):
        ...
        if mode is None:
            mode = self.mode
        super().add_text('{}/{}'.format(mode, tag), text_string, global_step, walltime)
    ...
    def add_pr_curve(self, tag, labels, predictions, global_step=None, num_thresholds=127, weights=None, walltime=None,
        ...
        if mode is None:
            mode = self.mode
        super().add_pr_curve('{}/{}'.format(mode, tag), labels, predictions, global_step, num_thresholds, weights,
                             walltime)
    ...
    def add_pr_curve_raw(self, tag, true_positive_counts, false_positive_counts, true_negative_counts,
        ...
        if mode is None:
            mode = self.mode
        super().add_pr_curve_raw('{}/{}'.format(mode, tag), true_positive_counts, false_positive_counts,
